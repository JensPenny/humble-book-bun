import type { BookItem, BookRating, Bundle } from "../types";
import { UnmarshalBookItem } from "../types";
import { getUnifiedRatings } from "./unified-ratings";
...
export async function analyzeBundleUrl(url: string): Promise<{ bundle: Bundle, books: Array<BookItem & { rating: BookRating }> }> {
  ...
  const booksWithRatings = await getUnifiedRatings(books);
